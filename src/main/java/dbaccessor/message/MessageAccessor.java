...
public final class MessageAccessor {
    ...
    public ImmutableList<Message> getCreationTsOfLastSentMessageIdForChatsOfUser(long userId, ImmutableList<Long> listOfChatIdOfUser) {
        ...
        String sqlStatment = "SELECT ChatID, CreationTS FROM Message WHERE MessageID IN (SELECT LastSentMessageID FROM Chat WHERE ChatID IN UNNEST (@listOfChatIdOfUser) AND LastSentMessageID IS NOT NULL)";
